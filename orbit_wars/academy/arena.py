"""Arena — the training battleground wrapping the Kaggle orbit_wars environment."""
...
from .cartography import TurnBatch, encode_turn
from .rivals import OpponentPolicy
...
class OrbitWarsEnv:
    ...
    def reset(self, seed: int | None = None) -> TurnBatch:
        ...
        self.last_obs = _extract_observation(learner_state)
        self.last_opp_obs = _extract_observation(opponent_state)
    ...
    def step(self, player_action: list[list[float | int]]) -> StepResult:
        ...
        reward = _terminal_reward(player_state, opp_state) if done else 0.0
...
def _terminal_reward(player_state: Any, opp_state: Any) -> float:
    player_reward = _extract_reward(player_state)
    opponent_reward = _extract_reward(opp_state)
