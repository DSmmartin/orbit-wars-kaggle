"""Rivals — opponent policies that the learner trains against in the arena."""
...
class NearestPlanetSniperOpponent:
    """Wraps the nearest-planet sniper as a stateless opponent.

    Intentionally stateless (previous_state=None on every call) so a single
    instance can be safely shared across multiple parallel training envs without
    cross-env state corruption.
    """

    def act(self, observation: Any) -> list[list[float | int]]:
        from orbit_wars.agents.nearest_planet_sniper import SniperPolicyConfig, build_moves, choose_shot_decisions
...
class SelfPlayOpponent:
    def __init__(self, cfg: TrainConfig, device: torch.device, deterministic: bool = True) -> None:
        from .cartography import candidate_feature_dim, global_feature_dim, self_feature_dim
